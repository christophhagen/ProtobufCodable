...
## Automatically hash string keys

Integer keys can greatly reduce the binary size of encoded objects, but must be specified for each object. A good trade-off can be to automatically hash the string keys into small integers when encoding fields, so that messages sizes are reduced. This could be an encoding option in the future. 
